...
namespace Tgirrr.CadLisp
{
    public sealed class LinetypeWindow : Window
    {
        ...
        void Create(object sender, RoutedEventArgs e)
        {
            try
            {
                var doc = Gssoft.Gscad.ApplicationServices.Core.Application.DocumentManager.MdiActiveDocument;
        ...
        void LoadDrawing(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                var options = new Gssoft.Gscad.EditorInput.PromptEntityOptions(
                    "\nChọn Pline hoặc đối tượng có nét cần chỉnh sửa: ");
                var picked = doc.Editor.GetEntity(options);
                if (picked.Status != Gssoft.Gscad.EditorInput.PromptStatus.OK) return;

                using (doc.LockDocument())
                using (var transaction = doc.Database.TransactionManager.StartTransaction())
                {
                    var entity = transaction.GetObject(picked.ObjectId,
                        Gssoft.Gscad.DatabaseServices.OpenMode.ForRead) as
                        Gssoft.Gscad.DatabaseServices.Entity;
                    if (entity != null)
                    {
                        typeName = entity.Linetype;
                        if (typeName.Equals("ByLayer", StringComparison.OrdinalIgnoreCase))
                        {
                            var layers = (Gssoft.Gscad.DatabaseServices.LayerTable)transaction.GetObject(
                                doc.Database.LayerTableId, Gssoft.Gscad.DatabaseServices.OpenMode.ForRead);
                            var layer = (Gssoft.Gscad.DatabaseServices.LayerTableRecord)transaction.GetObject(
                                layers[entity.Layer], Gssoft.Gscad.DatabaseServices.OpenMode.ForRead);
                            var record = (Gssoft.Gscad.DatabaseServices.LinetypeTableRecord)transaction.GetObject(
                                layer.LinetypeObjectId, Gssoft.Gscad.DatabaseServices.OpenMode.ForRead);
