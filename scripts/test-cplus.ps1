...
$a=Get-FileHash (Join-Path $root 'src\ribbon\bin\Release\net48\TGIRR.Ribbon.dll');$b=Get-FileHash (Join-Path $root 'bundle\TGIRR_CAD_LISP.bundle\Contents\Windows\TGIRR.Ribbon.dll');if($a.Hash-ne$b.Hash){throw 'DLL hash mismatch'};Write-Host "PASS: DLL SHA256 $($a.Hash)" -ForegroundColor Green
